...
class IDataProcessor : public IModule {
  public:
     ...
     */
    virtual ErrorCode processPacket(const RawDataPacketPtr &inputPacket, ProcessingResultPtr &result) = 0;
     ...
     */
    virtual std::future<ProcessingResultPtr> processPacketAsync(const RawDataPacketPtr &inputPacket) = 0;
     ...
     */
    virtual ErrorCode processBatch(const std::vector<RawDataPacketPtr> &inputPackets,
...
class ITaskScheduler : public IModule {
  public:
     ...
     */
    virtual std::future<ProcessingResultPtr> submitTaskWithResult(TaskWithResult task,
     ...
     * @return 处理结果的future对象
     */
    virtual std::future<ProcessingResultPtr> submitProcessingTask(std::shared_ptr<IDataProcessor> processor,
...
class IDisplayController : public IModule {
  public:
     ...
     * @return 操作结果错误码
     */
    virtual ErrorCode saveToFile(const std::string &filename, DisplayFormat format = DisplayFormat::FILE_CSV) = 0;
