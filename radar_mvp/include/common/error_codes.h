...
namespace radar
{
    ...
    public:
         ...
         */
        RadarException(ErrorCode errorCode,
            ...
            : errorCode_(errorCode), customMessage_(message), fileName_(fileName ? fileName : ""), lineNumber_(lineNumber)
        {
            ...
            oss << "RadarException [0x" << std::hex << errorCode_ << "]: "
                << getErrorDescription(errorCode_);
    ...
    {
    public:
        ModuleException(ErrorCode errorCode,
                        const std::string &message = "",
                        const char *fileName = nullptr,
                        int lineNumber = 0)
            : RadarException(errorCode, message, fileName, lineNumber) {}
...
/// 抛出雷达异常的便捷宏
#define RADAR_THROW(errorCode, message) \
    throw radar::RadarException(errorCode, message, __FILE__, __LINE__)

/// 抛出系统异常的便捷宏
#define SYSTEM_THROW(errorCode, message) \
    throw radar::SystemException(errorCode, message, __FILE__, __LINE__)

/// 抛出模块异常的便捷宏
#define MODULE_THROW(errorCode, message) \
    throw radar::ModuleException(errorCode, message, __FILE__, __LINE__)
