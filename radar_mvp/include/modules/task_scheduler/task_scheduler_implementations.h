...
class TaskScheduler : public ITaskScheduler {
  ...
  public:
     ...
     */
    std::future<ProcessingResultPtr> submitTaskWithResult(TaskWithResult task,
     ...
     */
    std::future<ProcessingResultPtr> submitProcessingTask(std::shared_ptr<IDataProcessor> processor,
  ...
  protected:
    ...
    std::unordered_map<ScheduledTask::TaskId, std::promise<ProcessingResultPtr>>
        resultPromises_;  ///< 结果Promise映射表
};
...
class ThreadPoolScheduler : public TaskScheduler {
  public:
     ...
     * @param logger 日志记录器实例
     */
    explicit ThreadPoolScheduler(uint32_t threadCount = 0, std::shared_ptr<spdlog::logger> logger = nullptr);
