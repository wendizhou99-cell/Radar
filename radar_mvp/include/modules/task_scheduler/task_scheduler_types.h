...
namespace radar
{
    ...
    public:
         ...
         * @param name 任务名称
         */
        ScheduledTask(TaskFunction taskFunction,
        ...
        // Getters
        TaskId getId() const { return taskId_; }
        const std::string &getName() const { return name_; }
        PacketPriority getPriority() const { return priority_; }
        TaskState getState() const { return state_.load(); }
        std::chrono::system_clock::time_point getSubmitTime() const { return submitTime_; }
        std::chrono::system_clock::time_point getStartTime() const { return startTime_; }
        std::chrono::system_clock::time_point getFinishTime() const { return finishTime_; }
        uint32_t getTimeoutMs() const { return timeoutMs_; }

        // Setters
        void setPriority(PacketPriority priority) { priority_ = priority; }
