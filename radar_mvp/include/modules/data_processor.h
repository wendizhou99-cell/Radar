...
class DataProcessor : public IDataProcessor {
  ...
  public:
     ...
     * @return 操作结果错误码
     */
    ErrorCode processPacket(const RawDataPacketPtr &inputPacket, ProcessingResultPtr &result) override;
     ...
     */
    std::future<ProcessingResultPtr> processPacketAsync(const RawDataPacketPtr &inputPacket) override;
  ...
  protected:
     ...
     */
    void enqueueTask(const RawDataPacketPtr &packet, std::promise<ProcessingResultPtr> &&promise);
     ...
     * @return 是否成功取出任务
     */
    bool dequeueTask(RawDataPacketPtr &packet, std::promise<ProcessingResultPtr> &promise, uint32_t timeoutMs = 1000);
};
...
class CPUDataProcessor : public DataProcessor {
  ...
  private:
     ...
     * @return 操作结果错误码
     */
    ErrorCode performFFT(const AlignedComplexVector &inputData, AlignedComplexVector &outputData);
     ...
     * @return 操作结果错误码
     */
    ErrorCode performFiltering(const AlignedComplexVector &inputData, AlignedComplexVector &outputData);
     ...
     * @return 操作结果错误码
     */
    ErrorCode performDetection(const AlignedComplexVector &inputData, std::vector<double> &detectionResults);
 ...
 */
std::unique_ptr<CPUDataProcessor> createCPUProcessor(const DataProcessorConfig &config,
 ...
 */
std::unique_ptr<GPUDataProcessor> createGPUProcessor(const DataProcessorConfig &config,
                                                     std::shared_ptr<spdlog::logger> logger = nullptr);
 ...
 */
std::unique_ptr<DataProcessor> createProcessor(ProcessorType processorType, const DataProcessorConfig &config,
                                               std::shared_ptr<spdlog::logger> logger = nullptr);
