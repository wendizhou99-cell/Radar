...
class PerformanceMonitor {
  ...
  private:
     ...
     * @return 移动平均值
     */
    std::vector<double> calculateMovingAverage(const std::vector<double> &values, size_t windowSize) const;
};
...
class StatisticsManager {
  public:
     ...
     * @return 操作结果错误码
     */
    ErrorCode registerController(const std::string &name, std::shared_ptr<PerformanceMonitor> monitor);
     ...
     * @return 统计信息指针
     */
    std::shared_ptr<const DisplayStatistics> getControllerStatistics(const std::string &name) const;
     ...
     * @return 操作结果错误码
     */
    ErrorCode exportStatistics(const std::string &filePath, const std::string &format = "CSV") const;

    /**
     * @brief 设置统计更新回调
     * @param callback 回调函数
     */
    void setStatisticsUpdateCallback(std::function<void(const std::string &, const DisplayStatistics &)> callback);
