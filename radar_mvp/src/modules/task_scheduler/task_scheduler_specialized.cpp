...
ThreadPoolScheduler::ThreadPoolScheduler(uint32_t threadCount, std::shared_ptr<spdlog::logger> logger)
...
RealTimeScheduler::RealTimeScheduler(std::shared_ptr<spdlog::logger> logger) : TaskScheduler(logger) {
...
void RealTimeScheduler::setRealTimeParams(uint32_t maxLatencyMs, bool preemptionEnabled) {
    ...
    RADAR_INFO("Real-time parameters set: maxLatency={}ms, preemption={}", maxLatencyMs, preemptionEnabled);
}
...
ErrorCode RealTimeScheduler::executeTask(const ScheduledTaskPtr &task) {
    ...
    if (maxLatencyMs_ > 0) {
        auto elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(startTime - std::chrono::steady_clock::now()).count();
        if (elapsed > maxLatencyMs_) {
            RADAR_WARN("Task {} exceeded max latency {}ms", task->getId(), maxLatencyMs_.load());
        }
    }
...
bool RealTimeScheduler::shouldPreempt(const ScheduledTaskPtr &newTask, const ScheduledTaskPtr &runningTask) const {
    if (!newTask || !runningTask) {
        return false;
    }

    // 基于优先级的抢占逻辑
    return static_cast<int>(newTask->getPriority()) > static_cast<int>(runningTask->getPriority());
}
