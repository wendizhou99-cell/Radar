...
HardwareDataReceiver::HardwareDataReceiver(std::shared_ptr<spdlog::logger> logger) : DataReceiver(logger) {
    if (logger_) {
...
ErrorCode HardwareReceiver::initialize() {
    ...
    try {
        ...
        // 创建接收线程池
        receiverThreadPool_ =
            std::make_unique<ThreadPool>(config_.simulationEnabled ? 1 : 2  // 模拟模式1个线程，硬件模式2个线程
            );
...
ErrorCode HardwareReceiver::start() {
    ...
    if (state_ != ModuleState::READY && state_ != ModuleState::PAUSED) {
        MODULE_ERROR(DataReceiver, "Cannot start from state: {}", static_cast<int>(state_.load()));
...
ErrorCode HardwareReceiver::receivePacket(RawDataPacketPtr &packet, uint32_t timeoutMs) {
    ...
    // 等待数据或超时
    auto waitResult = bufferNotEmpty_.wait_for(lock, std::chrono::milliseconds(timeoutMs),
                                               [this] { return !dataBuffer_.empty() || shouldStop_; });
...
void HardwareReceiver::setPacketReceivedCallback(std::function<void(RawDataPacketPtr)> callback) {
...
ErrorCode HardwareReceiver::flushBuffer() {
    ...
    MODULE_WARN(DataReceiver, "Flushing buffer, {} packets will be dropped", dataBuffer_.size());
...
RawDataPacketPtr HardwareReceiver::generateSimulatedPacket() {
    ...
    for (uint32_t ch = 0; ch < packet->channelCount; ++ch) {
        generateChannelData(packet->iqData.data() + ch * packet->samplesPerChannel, packet->samplesPerChannel, ch);
    }
...
void HardwareReceiver::addTargetEcho(ComplexFloat *data, size_t samples, uint32_t channel,
    ...
    for (size_t i = 0; i < samples; ++i) {
        ...
        if (t >= delay && t < delay + simulationParams_.pulseWidth) {
            ...
            // 添加到信号
            data[i] += ComplexFloat(amplitude * std::cos(phase), amplitude * std::sin(phase));
        }
    }
}
...
void HardwareReceiver::addClutter(ComplexFloat *data, size_t samples) {
    ...
    for (size_t i = 0; i < samples; ++i) {
        ...
        data[i] += ComplexFloat(clutterAmp * std::cos(phase), clutterAmp * std::sin(phase));
    }
}
...
ErrorCode HardwareReceiver::initializeBuffer() {
    MODULE_INFO(DataReceiver, "Initializing receive buffer with size {}", config_.maxQueueSize);
...
bool HardwareReceiver::pushToBuffer(RawDataPacketPtr packet) {
    ...
    if (dataBuffer_.size() >= config_.maxQueueSize) {
        ...
        if (config_.overflowPolicy == "drop_oldest") {
            ...
            // 等待缓冲区有空间
            bufferNotFull_.wait(lock, [this] { return dataBuffer_.size() < config_.maxQueueSize || shouldStop_; });
    ...
    while (!shouldStop_) {
        ...
        try {
            ...
            if (config_.simulationEnabled) {
                ...
                // 模拟数据生成速率控制
                std::this_thread::sleep_for(std::chrono::milliseconds(config_.generationIntervalMs));
            } else {
                // 硬件模式：从硬件读取数据
...
void HardwareReceiver::updatePerformanceMetrics() {
    ...
    if (performanceMonitor_.lastReceiveTime != Timestamp{}) {
        auto latency =
            std::chrono::duration_cast<std::chrono::microseconds>(now - performanceMonitor_.lastReceiveTime).count();

        performanceMonitor_.averageLatencyUs = (performanceMonitor_.averageLatencyUs * 0.9) + (latency * 0.1);
    }
...
void HardwareReceiver::logPerformanceMetrics() {
    ...
    MODULE_INFO(DataReceiver,
                ...
                performanceMonitor_.packetsPerSecond, performanceMonitor_.averageLatencyUs, status.currentSize,
                status.totalCapacity, status.totalReceived, status.totalDropped);
}

double HardwareReceiver::calculateThroughput() {
    auto now = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(now - performanceMonitor_.startTime).count();
...
void HardwareReceiver::checkBufferHealth() {
    ...
    if (status.totalReceived > 0) {
        float dropRate = static_cast<float>(status.totalDropped) / (status.totalReceived + status.totalDropped);

        if (dropRate > 0.01f) {  // 1%丢包率阈值
...
void HardwareReceiver::setState(ModuleState newState) {
    ...
    if (oldState != newState) {
        MODULE_DEBUG(DataReceiver, "State transition: {} -> {}", static_cast<int>(oldState),
