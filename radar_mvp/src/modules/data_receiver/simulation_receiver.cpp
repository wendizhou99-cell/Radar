...
namespace radar
{
    namespace modules
    {
        ...
        SimulationDataReceiver::SimulationDataReceiver(std::shared_ptr<spdlog::logger> logger)
            : DataReceiver(logger)
        {
            // 初始化完成，空目标列表
        }
        ...
        void SimulationDataReceiver::addSimulationTarget(double range, double velocity, double rcs,
                                                         double azimuth, double elevation)
        {
            ...
            RADAR_INFO("Added simulation target: range={:.1f}m, velocity={:.1f}m/s, rcs={:.2f}m², azimuth={:.1f}°, elevation={:.1f}°",
