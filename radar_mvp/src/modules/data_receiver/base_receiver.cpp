...
ErrorCode DataReceiver::receivePacket(RawDataPacketPtr &packet, uint32_t timeoutMs) {
    ...
    if (timeoutMs == 0) {
        // 无限等待
        packetAvailable_.wait(lock, [this] { return !packetQueue_.empty() || shouldStop_.load(); });
    } else {
        // 有超时等待
        auto timeout = std::chrono::milliseconds(timeoutMs);
        if (!packetAvailable_.wait_for(lock, timeout, [this] { return !packetQueue_.empty() || shouldStop_.load(); })) {
...
std::future<RawDataPacketPtr> DataReceiver::receivePacketAsync() {
    ...
    std::thread([this, promise]() {
        ...
        if (result == SystemErrors::SUCCESS && packet) {
            ...
            promise->set_exception(std::make_exception_ptr(std::runtime_error("Failed to receive packet")));
        }
    }).detach();
...
ErrorCode DataReceiver::start() {
    ...
    try {
        ...
        // 启动接收线程
        receptionThread_ = std::thread([this]() { receptionLoop(); });
