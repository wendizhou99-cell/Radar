...
namespace radar
{
    namespace modules
    {
        ...
        namespace DataReceiverFactory
        {
            ...
            std::unique_ptr<UDPDataReceiver> createUDPReceiver(
                ...
                std::shared_ptr<spdlog::logger> logger)
            {
                try
                {
                    ...
                    if (configResult != SystemErrors::SUCCESS)
                    {
                        if (logger)
                        {
                            RADAR_ERROR("Failed to configure UDP receiver: error code 0x{:X}",
            ...
            std::unique_ptr<FileDataReceiver> createFileReceiver(
                const DataReceiverConfig &config,
                std::shared_ptr<spdlog::logger> logger)
            {
                try
            ...
            std::unique_ptr<HardwareDataReceiver> createHardwareReceiver(
                const DataReceiverConfig &config,
                std::shared_ptr<spdlog::logger> logger)
            {
                try
            ...
            std::unique_ptr<SimulationDataReceiver> createSimulationReceiver(
                const DataReceiverConfig &config,
                std::shared_ptr<spdlog::logger> logger)
            {
                // 完整实现
        ...
        bool ReceiverManager::registerReceiver(const std::string &name,
        ...
        size_t ReceiverManager::startAllReceivers()
        {
            ...
            for (auto &[name, receiver] : receivers_)
            {
                if (receiver)
                {
                    ...
                    {
                        RADAR_ERROR("Failed to start receiver '{}': error code 0x{:X}",
        ...
        size_t ReceiverManager::stopAllReceivers()
        {
            ...
            for (auto &[name, receiver] : receivers_)
            {
                if (receiver)
                {
                    ...
                    {
                        RADAR_ERROR("Failed to stop receiver '{}': error code 0x{:X}",
                                    name, static_cast<uint32_t>(result));
                    }
                }
            }
