...
namespace radar
{
    namespace modules
    {
        ...
        HybridDisplayController::HybridDisplayController()
            : DisplayControllerBase("HybridDisplayController"),
              subControllersMutex_(),
              subControllers_()
        {
                                                            ...
                                                            const std::string &name)
        {
            ...
            auto it = std::find_if(subControllers_.begin(), subControllers_.end(),
                                   [&name](const SubControllerInfo &info)
                                   {
        ...
        ErrorCode HybridDisplayController::initializeDisplay()
        {
            ...
            try
            {
                ErrorCode result = executeOnSubControllers([](DisplayControllerBase &controller)
        ...
        std::vector<radar::IDisplayController::DisplayFormat> HybridDisplayController::getSpecificSupportedFormats() const
        {
            ...
            return {
                ...
                radar::IDisplayController::DisplayFormat::FILE_CSV,
                radar::IDisplayController::DisplayFormat::FILE_JSON,
                radar::IDisplayController::DisplayFormat::GRAPHICS_2D,
                radar::IDisplayController::DisplayFormat::GRAPHICS_3D};
        }

        ErrorCode HybridDisplayController::saveDisplayToFile(const std::string &filePath,
            ...
            try
            {
                ...
                for (const auto &info : subControllers_)
                {
                    if (info.enabled && info.controller)
                    {
                        ...
                        bool supportsFile = std::any_of(formats.begin(), formats.end(),
                        ...
                        if (supportsFile)
                        {
                            ErrorCode result = info.controller->saveToFile(filePath, radar::IDisplayController::DisplayFormat::FILE_CSV);
                            if (result == SystemErrors::SUCCESS)
                            {
            ...
            for (auto &info : subControllers_)
            {
                if (info.enabled && info.controller)
                {
                    try
                    {
                        ...
                        if (result != SystemErrors::SUCCESS)
                        {
                            RADAR_WARN("Operation failed on sub-controller '{}': {}",
                                       info.name, static_cast<int>(result));
