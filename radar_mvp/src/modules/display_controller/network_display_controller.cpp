...
ErrorCode NetworkDisplayController::saveDisplayToFile(const std::string &filePath, const DisplayData &data) {
    // 标记未使用的参数以避免编译警告
    (void)filePath;
    (void)data;
...
ErrorCode NetworkDisplayController::broadcastToClients(const std::string &data) {
    ...
    try {
        ...
        if (successCount > 0) {
            RADAR_DEBUG("Successfully sent data to {} clients, {} failures", successCount, failureCount);
        }
...
std::string NetworkDisplayController::formatNetworkData(const DisplayData &data) {
    ...
    oss << "DURATION:" << std::fixed << std::setprecision(2) << result.statistics.processingDurationMs << "\n";
    oss << "CPU_USAGE:" << std::fixed << std::setprecision(1) << result.statistics.cpuUsagePercent << "\n";
    oss << "GPU_USAGE:" << std::fixed << std::setprecision(1) << result.statistics.gpuUsagePercent << "\n";
