...
namespace radar
{
    namespace modules
    {
        ...
        ErrorCode FileDisplayController::saveDisplayToFile(const std::string &filePath,
        ...
        std::string FileDisplayController::generateFilePath(uint32_t index)
        {
            ...
            oss << fileConfig_.outputDirectory << "/"
                << fileConfig_.filePrefix << "_"
                << std::put_time(&tm, "%Y%m%d_%H%M%S") << "_"
                << std::setfill('0') << std::setw(3) << index
                << fileConfig_.fileExtension;
