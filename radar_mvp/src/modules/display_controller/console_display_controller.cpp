...
ConsoleDisplayController::ConsoleDisplayController()
    : DisplayControllerBase("ConsoleDisplayController"), displayedLines_(0) {
...
std::vector<radar::IDisplayController::DisplayFormat> ConsoleDisplayController::getSpecificSupportedFormats() const {
    return {radar::IDisplayController::DisplayFormat::CONSOLE_TEXT,
            radar::IDisplayController::DisplayFormat::CONSOLE_CHART};
}

ErrorCode ConsoleDisplayController::saveDisplayToFile(const std::string &filePath, const DisplayData &data) {
    try {
        // 打开文件进行追加写入
...
std::string ConsoleDisplayController::formatAsText(const DisplayData &data) {
    ...
    oss << colorizeText("Processing Time: ", 32)
        << std::chrono::duration_cast<std::chrono::milliseconds>(result.processingTime.time_since_epoch()).count()
        << " ms\n";
    oss << colorizeText("Success: ", 32) << (result.processingSuccess ? "Yes" : "No") << "\n";
    oss << colorizeText("Processing Duration: ", 32) << std::fixed << std::setprecision(2)
...
std::string ConsoleDisplayController::formatAsChart(const DisplayData &data) {
    ...
    oss << createBarChart("Memory Usage", static_cast<float>(result.statistics.memoryUsageBytes) / (1024.0f * 1024.0f),
                          100.0f, 32);
