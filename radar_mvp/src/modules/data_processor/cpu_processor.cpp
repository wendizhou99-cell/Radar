 ...
 */
CPUDataProcessor::CPUDataProcessor(std::shared_ptr<spdlog::logger> logger) : DataProcessor(logger) {
...
ProcessingResultPtr CPUDataProcessor::executeProcessing(const RawDataPacketPtr &inputPacket) {
    MODULE_DEBUG(CPUDataProcessor, "Executing CPU processing for packet {}", inputPacket->sequenceId);
    ...
    try {
        ...
        if (inputPacket->channelCount > 1) {
            ...
            std::transform(beamformedData.begin(), beamformedData.end(), result->beamformedData.begin(),
                           [](const ComplexFloat &c) { return std::abs(c); });
        }

        // 填充处理结果 - 转换double到float
        result->rangeProfile.resize(detectionResults.size());
        std::transform(detectionResults.begin(), detectionResults.end(), result->rangeProfile.begin(),
                       [](double d) { return static_cast<float>(d); });

        // 计算多普勒频谱（示例实现）
        result->dopplerSpectrum.resize(frequencyData.size());
        std::transform(frequencyData.begin(), frequencyData.end(), result->dopplerSpectrum.begin(),
                       [](const ComplexFloat &c) { return std::abs(c); });
 ...
 * @todo 实现零填充和重叠处理优化
 */
ErrorCode CPUDataProcessor::performFFT(const AlignedComplexVector &inputData, AlignedComplexVector &outputData) {
 ...
 * @todo 实现多级滤波器组用于信道分离
 */
ErrorCode CPUDataProcessor::performFiltering(const AlignedComplexVector &inputData, AlignedComplexVector &outputData) {
