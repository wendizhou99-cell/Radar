...
ProcessorCapabilities GPUDataProcessor::getCapabilities() const {
    ...
    caps.supportedStrategies = {ProcessingStrategy::CPU_BASIC, ProcessingStrategy::GPU_ACCELERATED,
...
ProcessingResultPtr GPUDataProcessor::executeProcessing(const RawDataPacketPtr &inputPacket) {
    MODULE_DEBUG(GPUDataProcessor, "Executing GPU processing for packet {}", inputPacket->sequenceId);
    ...
    try {
        // 根据当前策略选择处理方式
        bool useGPU = (currentStrategy_ == ProcessingStrategy::GPU_ACCELERATED) && (gpuContext_ != nullptr);
        ...
        if (result) {
            ...
            MODULE_DEBUG(GPUDataProcessor, "GPU processing completed in {:.3f}ms ({})", duration,
                         useGPU ? "GPU" : "CPU");
        }
    } catch (const std::exception &e) {
...
ErrorCode GPUDataProcessor::initializeGPU() {
    ...
    try {
        ...
        MODULE_INFO(GPUDataProcessor, "GPU initialized successfully: device {}, memory {}MB", deviceId_,
                    deviceMemorySize_ / (1024 * 1024));
        return SystemErrors::SUCCESS;
    } catch (const std::exception &e) {
...
ProcessingResultPtr GPUDataProcessor::processOnGPU(const RawDataPacketPtr &inputPacket) {
    ...
    try {
        ...
        if (dataSize > deviceMemorySize_) {
            MODULE_ERROR(GPUDataProcessor, "Input data size {} exceeds GPU memory {}", dataSize, deviceMemorySize_);
            result->processingSuccess = false;
            return result;
        }
...
ErrorCode GPUDataProcessor::copyToDevice([[maybe_unused]] const void *hostData, [[maybe_unused]] void *devicePtr,
...
ErrorCode GPUDataProcessor::copyFromDevice([[maybe_unused]] const void *devicePtr, [[maybe_unused]] void *hostData,
                                           [[maybe_unused]] size_t size) {
#ifdef CUDA_ENABLED
...
ErrorCode GPUDataProcessor::performGPUFFT([[maybe_unused]] void *inputData, [[maybe_unused]] void *outputData,
                                          [[maybe_unused]] size_t size) {
#ifdef CUDA_ENABLED
    // TODO: 实现真正的cuFFT调用
    // 当前为框架实现，直接复制数据
