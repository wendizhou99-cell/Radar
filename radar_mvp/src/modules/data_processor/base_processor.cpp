...
namespace radar
{
    ...
    DataProcessor::DataProcessor(std::shared_ptr<spdlog::logger> logger)
        : logger_(logger ? logger : common::LoggerManager::getInstance().getLogger("DataProcessor")), currentStrategy_(ProcessingStrategy::CPU_BASIC)
    {
        statistics_.reset();

        MODULE_INFO(DataProcessor, "DataProcessor created with strategy: {}",
                    static_cast<int>(currentStrategy_));
    }
    ...
    ErrorCode DataProcessor::configure(const DataProcessorConfig &config)
    {
        ...
        MODULE_INFO(DataProcessor, "Configuring processor with strategy: {}",
    ...
    ErrorCode DataProcessor::processPacket(const RawDataPacketPtr &inputPacket,
     ...
     */
    std::future<ProcessingResultPtr> DataProcessor::processPacketAsync(
        const RawDataPacketPtr &inputPacket)
    {
        // 验证处理器状态
        if (currentState_.load() != ModuleState::RUNNING)
        {
            ...
            promise.set_exception(std::make_exception_ptr(
                ModuleException(DataProcessorErrors::PROCESSOR_NOT_READY,
                                "Processor not in running state")));
        ...
        if (!validateInputPacket(inputPacket))
        {
            std::promise<ProcessingResultPtr> promise;
            promise.set_exception(std::make_exception_ptr(
                ModuleException(DataProcessorErrors::INVALID_INPUT_DATA,
     ...
     */
    ErrorCode DataProcessor::processBatch(
        ...
        std::vector<ProcessingResultPtr> &results)
    {
        ...
        {
            MODULE_WARN(DataProcessor, "Batch size {} exceeds maximum {}",
                        inputPackets.size(), MAX_BATCH_SIZE);
            return SystemErrors::INVALID_PARAMETER;
        }
        ...
        MODULE_INFO(DataProcessor, "Batch processing completed: {}/{} successful in {}ms",
                    successCount, inputPackets.size(), batchDuration);
    ...
    ErrorCode DataProcessor::switchStrategy(ProcessingStrategy strategy)
    {
        ...
        MODULE_INFO(DataProcessor, "Switching strategy from {} to {}",
                    static_cast<int>(currentStrategy_), static_cast<int>(strategy));
        ...
        try
        {
            // 清空任务队列
            {
                ...
                while (!taskQueue_.empty())
                {
                    ...
                    task.second.set_exception(std::make_exception_ptr(
                        ModuleException(SystemErrors::SHUTDOWN_FAILED, "System shutting down")));
                    taskQueue_.pop();
                }
            }
            ...
            while (!shouldStop_.load())
            {
                ...
                if (!running_.load())
                {
                    ...
                    taskAvailable_.wait(lock, [this]
                                        { return shouldStop_.load() || running_.load(); });
                ...
                try
                {
                    ...
                    else
                    {
                        // 处理返回空结果，设置异常供调用者处理
                        promise.set_exception(std::make_exception_ptr(
                            ModuleException(DataProcessorErrors::PROCESSING_FAILED,
    ...
    void DataProcessor::setState(ModuleState newState)
    {
        ...
        {
            MODULE_DEBUG(DataProcessor, "State changed: {} -> {}",
    ...
    void DataProcessor::enqueueTask(const RawDataPacketPtr &packet,
                                    std::promise<ProcessingResultPtr> &&promise)
    {
        ...
        if (taskQueue_.size() >= maxQueueSize)
        {
            ...
            throw ModuleException(TaskSchedulerErrors::TASK_QUEUE_FULL,
                                  "Processing task queue is full");
        }
    ...
    bool DataProcessor::dequeueTask(RawDataPacketPtr &packet,
                                    std::promise<ProcessingResultPtr> &promise,
                                    uint32_t timeoutMs)
    {
        std::unique_lock<std::mutex> lock(taskQueueMutex_);
        ...
        if (taskQueue_.empty())
        {
            ...
            // 条件等待：直到有任务入队或收到停止信号
            if (!taskAvailable_.wait_for(lock, timeout, [this]
