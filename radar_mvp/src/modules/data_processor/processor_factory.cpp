...
std::unique_ptr<CPUDataProcessor> createCPUProcessor(const DataProcessorConfig &config,
    ...
    try {
        ...
        if (configResult != SystemErrors::SUCCESS) {
            if (logger) {
                MODULE_ERROR(DataProcessorFactory, "Failed to configure CPU processor: {}", configResult);
            }
            return nullptr;
        }
...
std::unique_ptr<GPUDataProcessor> createGPUProcessor(const DataProcessorConfig &config,
    ...
    try {
        ...
        if (configResult != SystemErrors::SUCCESS) {
            if (logger) {
                MODULE_ERROR(DataProcessorFactory, "Failed to configure GPU processor: {}", configResult);
            }
            return nullptr;
        }
...
std::unique_ptr<DataProcessor> createProcessor(ProcessorType processorType, const DataProcessorConfig &config,
    ...
    switch (processorType) {
        ...
        default:
            if (logger) {
                MODULE_ERROR(DataProcessorFactory, "Unknown processor type: {}", static_cast<int>(processorType));
            }
            return nullptr;
    }
}
