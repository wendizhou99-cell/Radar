...
ErrorCode LoggerManager::initialize(const LoggerConfig &config) {
    ...
    try {
        ...
        RADAR_DEBUG("Async mode: {}, Queue size: {}, Thread pool size: {}", config_.asyncMode, config_.asyncQueueSize,
...
std::shared_ptr<spdlog::logger> LoggerManager::createModuleLogger(const std::string &moduleName, LogLevel level) {
    if (sinks_.empty()) {
    ...
    try {
        ...
        if (config_.asyncMode) {
            logger = std::make_shared<spdlog::async_logger>(
                moduleName, sinks_.begin(), sinks_.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block);
        } else {
            logger = std::make_shared<spdlog::logger>(moduleName, sinks_.begin(), sinks_.end());
        }
...
std::shared_ptr<spdlog::sinks::sink> LoggerManager::createFileSink() {
    try {
        auto sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
            config_.file.filename, config_.file.maxFileSize, config_.file.maxFiles);
