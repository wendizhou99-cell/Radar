...
namespace radar
{
    namespace common
    {
        ...
        uint32_t ConfigManager::registerChangeCallback(ConfigChangeCallback callback,
        ...
        std::string ConfigManager::exportToString(bool pretty) const
        {
            ...
            try
            {
                ...
                if (pretty)
                {
                    ...
                    oss << "# Generated at: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count() << "\n\n";
                }
                oss << rootConfig_;
                return oss.str();
            }
        ...
        YAML::Node ConfigManager::getNode(const std::string &keyPath, bool createIfNotExists)
        {
            ...
            if (!createIfNotExists)
            {
                ...
                std::function<YAML::Node(const YAML::Node &, const std::vector<std::string> &, size_t)>
                    getNodeRecursive = [&](const YAML::Node &node, const std::vector<std::string> &path, size_t index) -> YAML::Node
                {
                    if (index >= path.size())
                    {
                        return node;
