...
展示从UDP数据包接收到屏幕渲染的完整数据处理链路，这是系统的核心业务流程。
...
sequenceDiagram
    ...
    participant DisplayController as 显控接口模块
    participant UI as 用户界面

    Note over Radar,UI: 端到端数据处理流程 (Happy Path)
    ...
    Note right of DisplayController: 保持trace_id: trace-abc123用于调试
    DisplayController->>DisplayController: 12. 数据适配与ViewModel更新
    DisplayController->>UI: 13. 触发Qt界面重绘
    UI->>UI: 14. 渲染目标点到屏幕
    deactivate DisplayController

    Note over Radar,UI: 完整链路处理完成，Trace ID贯穿全程用于端到端追踪
```
...
1. **数据接收阶段（1-4）**：
   - 雷达阵面通过UDP发送原始I/Q数据包
   - DataReceiver执行数据包解析、验证和封装
   - **生成全局唯一Trace ID**，用于端到端追踪
   - 封装为DataObject推送到共享缓冲区

2. **信号处理阶段（5-7）**：
   - SignalProcessor从缓冲区获取DataObject
   - **继承Trace ID**，确保追踪链完整性
   - 执行GPU算法流水线（FFT、滤波、CFAR检测）
   - 生成DetectionResult推送到下游

3. **数据处理阶段（8-10）**：
   - DataProcessor获取检测结果
   - **保持Trace ID传递**，执行目标跟踪算法
   - 通过数据关联、航迹管理生成TrackData
   - 推送到显控模块缓冲区

4. **显示渲染阶段（11-14）**：
   - DisplayController获取航迹数据
   - **Trace ID用于调试和性能分析**
   - 数据适配后触发Qt界面重绘
   - 最终在屏幕上渲染目标点

**性能特征**：
- **端到端延迟**：< 20ms（从UDP包到屏幕显示）
- **零拷贝传递**：所有模块间通过智能指针传递，避免数据拷贝
- **完整追踪**：Trace ID贯穿全链路，支持性能分析和问题定位
...
展示显控模块与计算密集型模块之间的抢占式GPU资源协调机制，防止GPU超载导致的系统响应问题。
...
sequenceDiagram
    ...
    Note over User,GpuMonitor: 抢占式GPU资源协调流程

    %% 阶段1: 正常运行状态
    User->>DisplayController: 1. 用户进行复杂界面操作 (如缩放、旋转)
    activate DisplayController
    DisplayController->>GpuMonitor: 2. 查询GPU使用率
    GpuMonitor-->>DisplayController: 3. 当前GPU使用率: 92% (超过阈值85%)

    %% 阶段2: 资源冲突检测
    DisplayController->>DisplayController: 4. 检测到GPU资源紧张
    Note right of DisplayController: 风险: UI渲染延迟可能导致<br/>用户体验恶化或GPU驱动TDR

    DisplayController->>TaskScheduler: 5. 发送SET_COMPUTE_PRIORITY(LOW)事件
    Note right of DisplayController: trace_id: gpu-coord-abc123<br/>原因: GPU_OVERLOAD

    %% 阶段3: 抢占式优先级调度
    activate TaskScheduler
    TaskScheduler->>TaskScheduler: 6. 执行强制性优先级调度决策
    Note right of TaskScheduler: 策略: 保证UI实时性优先<br/>计算任务可适当降级

    par 并行向所有GPU消费模块广播
        TaskScheduler->>SignalProcessor: 7a. SET_COMPUTE_PRIORITY(LOW)
        TaskScheduler->>DataReceiver: 7b. SET_COMPUTE_PRIORITY(LOW)
    end

    %% 阶段4: 模块响应优先级变更
    SignalProcessor->>SignalProcessor: 8a. 中断当前CUDA Kernel执行
    activate SignalProcessor
    SignalProcessor->>SignalProcessor: 9a. 切换到LowPriorityComputeStream
