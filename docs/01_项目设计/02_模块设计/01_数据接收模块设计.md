...
| 中文术语       | 英文术语             | 代码标识符         | 架构层次   | 技术说明                              |
| -------------- | -------------------- | ------------------ | ---------- | ------------------------------------- |
| 中间队列       | Raw Packet Queue     | `raw_packet_queue` | 解耦缓冲层 | 连接I/O线程和工作线程池的SPSC无锁队列 |
| 最终环形缓冲区 | Final Ring Buffer    | `raw_data_buffer`  | 共享缓冲层 | 与信号处理模块共享的环形缓冲区        |
| 数据包解析器   | Packet Parser        | `IPacketParser`    | 并行处理层 | 策略接口，用于可插拔的数据包解析      |
| 数据验证器     | Data Validator       | `IPacketValidator` | 并行处理层 | 策略接口，用于可插拔的数据完整性检查  |
| 页锁定内存池   | Pinned Memory Pool   | `memory_pool_`     | 控制管理层 | 预分配的GPU友好内存池                 |
| 统计收集器     | Statistics Collector | `stats_collector_` | 控制管理层 | 多维度错误统计和性能监控组件          |
| I/O接收线程    | IO Thread            | `io_thread_`       | I/O接收层  | 专用网络I/O线程，执行epoll事件循环    |
| 解析工作线程池 | Worker Thread Pool   | `worker_pool_`     | 并行处理层 | CPU密集型任务处理线程池               |
| 事件总线       | Event Bus            | `event_bus_`       | 系统集成层 | 模块间异步通信机制                    |
| 背压机制       | Backpressure         | N/A                | 流量控制   | 基于阻塞队列的隐式流量控制机制        |

> **说明**：本术语表按照系统架构层次分类，便于开发者理解各组件在整体架构中的位置和作用。
