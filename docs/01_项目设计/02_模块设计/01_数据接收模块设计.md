...
- [数据接收模块设计](#数据接收模块设计)
  ...
  - [8 性能优化策略](#8-性能优化策略)
    ...
    - [8.4 GPU资源协调与抢占式优先级管理](#84-gpu资源协调与抢占式优先级管理)
      ...
      - [8.4.3 任务切分与抢占响应机制](#843-任务切分与抢占响应机制)
...
```mermaid
sequenceDiagram
    participant Gateway as 数据网关模块
    participant Scheduler as 任务调度器
    participant DataReceiver as 数据接收模块
    participant SignalProcessor as 信号处理模块

    Note over Gateway,SignalProcessor: GPU负载过高场景下的抢占式资源协调

    Gateway->>Gateway: 1. 检测到GPU使用率 > 85%
    Gateway->>Scheduler: 2. 发送 SET_COMPUTE_PRIORITY(LOW) 事件
