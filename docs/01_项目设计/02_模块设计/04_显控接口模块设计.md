...
- [显控接口模块设计](#显控接口模块设计)
  ...
  - [3 Qt6.x架构设计](#3-qt6x架构设计)
    ...
    - [3.3 线程模型设计](#33-线程模型设计-1)
      - [3.3.1 线程模型改进：延迟更新机制](#331-线程模型改进延迟更新机制)
  - [4 界面组件架构](#4-界面组件架构)
...
public:
    ...
    // 通知CUDA模块降低计算优先级
    void requestCudaThrottling(bool enable, uint64_t trace_id);
...
2. **策略切换**: 根据GPU负载自动选择渲染质量，确保信号处理优先级
3. **上下文隔离**: 确保OpenGL和CUDA上下文完全独立，避免状态污染
4. **优雅降级**: 在GPU过载时，优先保证信号处理功能，显控功能降级
5. **Trace ID支持**: 所有GPU资源协调操作都支持端到端追踪
...
| 版本   | 日期       | 作者    | 变更描述                                                                                                                                                                                                   |
| ------ | ---------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v2.0.0 | 2025-09-24 | Kelin   | **重大架构修复**: 1) 添加高性能交互处理机制(四叉树空间索引); 2) 完善错误处理架构(错误码定义、状态管理、IModule集成); 3) 修正技术选型(移除QML，专注Qt Widgets); 4) 解决问题汇总中识别的所有11个关键架构问题 |
