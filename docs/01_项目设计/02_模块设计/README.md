...
| `DisplayController` (数据网关模块)                            | 作为后端服务器的数据出口，将处理完成的航迹数据通过UDP高效地分发给外部显控终端。         | 04_数据网关模块设计.md     |
...
| **独立客户端应用 (Standalone Client Application)**            |                                                                                         |                            |
| `显控终端应用`                                                | 一个独立的桌面应用，作为人机交互界面(HMI)，负责实时数据可视化和对后端服务器的远程控制。 | 09_显控终端应用设计.md     |
...
#### 1.4 `DisplayController` (数据网关模块)
作为后端服务器与外部世界的数据桥梁，`DisplayController` 的角色已从UI界面演变为一个纯粹的、无界面的“数据快递员”。
- **核心职责**: 消费内部处理完成的`TrackData`，使用**Protobuf**进行高效序列化，并通过**UDP单播**将其分发到配置好的显控终端地址。
- **架构定位**: 它是数据处理流水线的最后一环，是**数据面**的出口，完全遵循**数据与控制分离**原则。
- **性能与解耦**: 采用单工作线程的流水线模型，确保数据发送的顺序性，并通过事件总线与系统的其他部分保持低耦合。
...
#### 3.1 `显控终端应用`
在新的分布式架构下，这是一个完全独立的桌面应用程序，是操作员与后端服务器交互的唯一官方图形界面。
- **核心架构**: 基于 **Qt6** 框架和以 **ViewModel为中心** 的多线程架构，将UI渲染与网络通信、业务逻辑彻底分离，确保界面在高数据负载下依然流畅。
- **双通道通信**: 严格遵循“数据与控制分离”原则，通过**RESTful API**（控制面）发送命令和查询状态，同时通过**UDP监听器**（数据面）接收实时航迹数据。
- **健壮性设计**: 内置网络状态感知和**指数退避重连**机制，能够在不稳定的网络环境中保持稳定运行并尝试自动恢复。
