...
本文件是雷达数据处理系统所有接口设计的**最高层级索引和指导性纲领**。其核心目标是提供一个清晰的接口架构视图，并为团队成员定义一套统一的接口设计原则和文档编写路线图。它将确保所有接口设计在理念上保持一致，在实现上无缝集成。

-----

## 目录

- [00\_接口设计总览](#00_接口设计总览)
  - [概述](#概述)
  - [目录](#目录)
  - [1 文档职责](#1-文档职责)
...
graph TD
    subgraph " "
        BusinessModule["<b>业务模块</b><br/>(DataReceiver, SignalProcessor,<br/>DataProcessor, <b>DisplayController</b>)"]
        TaskScheduler["任务调度器"]
        EventBus(("EventBus"))
        DataPipeline["数据处理管道"]
        ExternalFacade["<b>外部接口网关</b><br/>(REST API Facade)"]
    end

    subgraph "外部世界 (External World)"
        ClientApp["<b>显控终端 / 第三方系统</b>"]
    end

    subgraph "模块控制接口"
        ILifecycle["ILifecycleManaged"]
    end
    subgraph "核心服务接口"
        ILogger["ILogger"]
    end
    ...
    subgraph "外部系统接口"
        RestAPI["RESTful API<br/>(HTTP)"]
        UdpAPI["实时数据协议<br/>(UDP)"]
    end

    %% 关系
    TaskScheduler -- "调用/管理" --> ILifecycle
    BusinessModule -- "实现" --> ILifecycle
    ILogger -.-> |"注入"| BusinessModule
    BusinessModule -. "发布/订阅" .-> EventBus
    EventBus -. "传递" .-> BaseEvent
    BusinessModule -- "通过...交换数据" --> DataPipeline
    DataPipeline -- "传递" --> DataPacket

    ExternalFacade -- "实现" --> RestAPI
    DisplayController -- "实现" --> UdpAPI

    ClientApp -- "<b>控制流</b>" --> RestAPI
    UdpAPI -- "<b>数据流</b>" --> ClientApp

    classDef internal fill:#e6f7ff,stroke:#1890ff,stroke-width:2px,color:#0050b3
    classDef external fill:#fffbe6,stroke:#d4b106,stroke-width:2px,color:#876800

    class BusinessModule,TaskScheduler,EventBus,DataPipeline,ExternalFacade internal
...
#### 3.2.5 外部系统接口 (External System Interfaces)
  ...
  - **核心职责**: 定义整个核心处理服务器如何与外部世界进行交互。这是系统“门面”(Facade)的正式契约。
  - **关键接口**:
      - **RESTful API**: 基于HTTP/TCP的**可靠控制通道**，用于外部客户端（如显控终端）查询系统状态、下发控制命令、修改配置。
      - **实时数据协议 (UDP)**: 基于UDP的**高效数据通道**，由`数据网关模块(DisplayController)`负责，用于向外部客户端单向、高速地推送处理后的数据。
  - **交互模式**: 外部客户端通过**两个独立的通道**与系统交互：通过RESTful API进行可靠的请求-响应，同时通过UDP监听器接收实时的、非保证送达的数据流。
